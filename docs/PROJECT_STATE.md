...
substantive wave was the **Executive Demo / UX wave** (`docs/EXECUTIVE_DEMO_REPORT.md`,
2026-09-07): full bilingual (EN/FA) technical panels with canonical identifiers preserved, the
global table-layout fix, simulated-live playback embedded in the OT document, one global demo
disclosure, and the focused regression module `tests/test_task6_executive_demo.py`; before that
the **Cement Plant OT Platform shell wave** (`docs/OT_PLATFORM_SHELL_REPORT.md`, 2026-09-06): one
consolidated, tabbed, management-facing HTML artifact (`reports/cement_plant_ot_platform.html`,
`--view OT`) reusing the existing renderers verbatim; before that the **Cell 6 resumable
training wave** (`docs/CELL6_RESUME_REPORT.md`, 2026-09-05): `src/notebook_support.py`
(per-dataset checkpoint/manifest/resume logic — zero frozen-layer changes) and notebook cell 6
rewritten to call it; before that the **PRD §25 Colab notebook**
...
| | |
|---|---|
| **Branch** | `main` — the Executive Demo / UX wave is the tip; see `git log` for the earlier waves. |
| **HEAD after this wave** | the Executive-Demo-wave commit, whose parent is the OT-Platform-shell-wave commit (`b71db2a`). Not pinned here: a commit cannot contain its own hash. |
| **Wave history** | `1f8107f` baseline → `0ed5e39` directive persisted → `3fa2e7d` Wave 1 → `e4dee7a` Wave 2 → `440602e` Wave 3A → `8cbda49` Wave 3B → `557b935` Wave 3C → `b2915e3` Wave 3C merge → Wave 3D (`6b27858` merge) → `a056bf9` item 15 reconstruction → Wave View J (`cac1296`) → Wave View J closeout (`89a93ff`) → Wave View J horizon (`963f6d2`) → View H audit (`52ac068`) → Wave View H (`4a70160`) → View H closeout (`cc86f54`) → Wave View A (`8f61802`) → View A closeout pin (`6dfb67b`) → Wave View G (`6ee2d56`) → Wave View I (`091cb4a`) → Wave CDF (`5795e5d`) → Wave View I transition chart (`e057125`) → Wave Item 17 Factory Presentation Mode (`1db6cec`) → final gap audit (`655cee1`) → PRD §25 notebook + Item 19 + §28 demos (`f585b50`) → Cell 6 resumable training (`33c5dad`) → OT Platform shell (`b71db2a`) → **Executive Demo / UX wave** |
| **Full regression** | **784 passed, 0 xfailed** (Executive Demo / UX wave, 2026-09-07: 765 + 19 tests of `tests/test_task6_executive_demo.py`; goldens regenerated with the recorded commands after the bilingual-renderer and single-disclosure changes). |
| **xfails** | **None.** |
...
no fabricated alerts)* · `test_task6_executive_demo.py` *(Executive Demo / UX wave — the six
directive guarantee groups over the real document: localization, RTL/LTR, table stability,
simulated-live playback, honesty, self-containedness)*
...
| **Executive Demo / UX wave** | **DONE** (2026-09-07) | The OT document's technical panels are now **fully bilingual**: `src/visualization/i18n.py` (new) centralizes the EN/FA pairs (`i18n.bi`, `tag_title`, `tag_ref`, `title_bi`, status/state dictionaries, the ten view titles + subtitles), and every renderer emits `dt-en`/`dt-fa` pairs switched by CSS on `<html dir>` — the same mechanism the shell's `otSetLang` writes, so one attribute write switches the whole document. Canonical identifiers are never renamed and appear in Persian mode only as the secondary `شناسه فنی: <bdi>…</bdi>` reference. The global table-layout fix (capped `minmax(min(…,100%),1fr)` floors, `min-width:0` cards, `overflow-wrap:anywhere` identifiers) landed in the shared stylesheet and the per-view layout CSS. **Simulated-live playback** (`src/visualization/playback.py`, new): the timeline is built at export time by render-then-diff — advancing the provider per tick, re-rendering the time-indexed views A–H through the same `build_view_section` dispatch, and diffing `data-otk`-anchored nodes; the runtime patches `[data-otk]` nodes (plain strings → `textContent`, `[en,fa,cls]` triples → language children + `dt-pill--*` class swap) with Play/Pause/Reset, 1×/2×/4×, a sim clock and a progress bar; no `Math.random`, no network, one inline script. Scenario-level results (J/I/P) stay static by rule. **One global demo disclosure** (header + footer, the directive's exact EN/FA wording) replaces the per-screen simulated/not-validated badges; the PRD-verbatim no-plant-connection banners and PRD 29 presentation card labels stay. Presentation is the landing tab with a bilingual three-step how-to. 19 focused tests + regenerated goldens; full regression 784 passed; both frozen digests unchanged. Browser automation unavailable — validation static + JS syntax check; see `docs/EXECUTIVE_DEMO_REPORT.md`. |
