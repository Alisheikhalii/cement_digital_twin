...
substantive wave was **Wave CDF** (`docs/VIEWCDF_AUDIT_AND_IMPLEMENTATION_REPORT.md`,
2026-09-03) — the one shared renderer for the three process detail screens (views C, D and F;
directive items 5, 6, 9 and the item-4 inspector half), which completes the renderer inventory:
**all ten A–J screens now have a renderer.**
...
| **HEAD after Wave CDF** | the Wave CDF commit, whose parent is the Wave View I commit (`091cb4a`). Not pinned here: a commit cannot contain its own hash. |
| **Wave history** | `1f8107f` baseline → `0ed5e39` directive persisted → `3fa2e7d` Wave 1 → `e4dee7a` Wave 2 → `440602e` Wave 3A → `8cbda49` Wave 3B → `557b935` Wave 3C → `b2915e3` Wave 3C merge → Wave 3D (`6b27858` merge) → `a056bf9` item 15 reconstruction → Wave View J (`cac1296`) → Wave View J closeout (`89a93ff`) → Wave View J horizon (`963f6d2`) → View H audit (`52ac068`) → Wave View H (`4a70160`) → View H closeout (`cc86f54`) → Wave View A (`8f61802`) → View A closeout pin (`6dfb67b`) → Wave View G (`6ee2d56`) → Wave View I (`091cb4a`) → **Wave CDF** |
| **Full regression** | **689 passed, 0 xfailed** (was 662; the CDF wave added 27 tests in the new `test_task6_process_view.py`, changed none). |
| **xfails** | **None.** |
...
shape fix documented in `VIEWI_AUDIT_AND_IMPLEMENTATION_REPORT.md` §8.6)* and
...
commands verified to run. §5's step-3 note and §9's table rows 5–6 still carry the old
"no `--change` flag" claim — docs backlog, flagged in
`VIEWCDF_AUDIT_AND_IMPLEMENTATION_REPORT.md` §6.5. |
