...
substantive wave was **Wave View I transition chart** (`docs/VIEWI_TRANSITION_CHART_REPORT.md`,
2026-09-03) — the PRD §16.2/§16.3 transition chart on view I as a self-contained inline SVG, plus
three documentation corrections (`TestNfr2Budget`, DEMO_GUIDE §5/§9, the C/D/F constraints
verdict). The renderer inventory it inherited from Wave CDF stands: **all ten A–J screens have a
renderer.**
...
| | |
|---|---|
| **Branch** | `main` — the Wave Item 17 (Factory Presentation Mode) commit is the tip; earlier waves 3C/3D were merged via `main` (see `git log`). |
| **HEAD after this wave** | the Wave Item 17 commit, whose parent is the Wave View I transition-chart commit (`e057125`). Not pinned here: a commit cannot contain its own hash. |
| **Wave history** | `1f8107f` baseline → `0ed5e39` directive persisted → `3fa2e7d` Wave 1 → `e4dee7a` Wave 2 → `440602e` Wave 3A → `8cbda49` Wave 3B → `557b935` Wave 3C → `b2915e3` Wave 3C merge → Wave 3D (`6b27858` merge) → `a056bf9` item 15 reconstruction → Wave View J (`cac1296`) → Wave View J closeout (`89a93ff`) → Wave View J horizon (`963f6d2`) → View H audit (`52ac068`) → Wave View H (`4a70160`) → View H closeout (`cc86f54`) → Wave View A (`8f61802`) → View A closeout pin (`6dfb67b`) → Wave View G (`6ee2d56`) → Wave View I (`091cb4a`) → Wave CDF (`5795e5d`) → Wave View I transition chart (`e057125`) → **Wave Item 17 Factory Presentation Mode** |
| **Full regression** | **715 passed, 0 xfailed** (was 693; this wave added 22 tests in `test_task6_presentation_view.py` + the `tests/golden/view_p_normal.html` fixture, changed none). |
| **xfails** | **None.** |
